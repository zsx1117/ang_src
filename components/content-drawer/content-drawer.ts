...
export class ContentDrawer {
  ...
  percentageLeftWhenUp: number = 35;
  percentageLeftWhenDown: number = 85;
  ...
  handlePan(ev){
    ...
    if(this.bounceBack && ev.isFinal){
      ...
      let topDiffForBounce = topDiff/6;
    ...
    if((newTop < this.thresholdTop && ev.additionalEvent === "panup") || bounceToTop){
      ...
      this.domCtrl.write(() => {
        this.renderer.setElementStyle(this.element.nativeElement, 'transition', 'top 1s');
        this.renderer.setElementStyle(this.element.nativeElement, 'top', ((this.platform.height() - this.handleHeight)*this.percentageLeftWhenUp/100) + 'px');
      });
